...
for i =1 :numRxpoints
% error_abs(i) = nmse(abs(H_UPA(:,:,i)),abs(Hinsite_NrNt(:,:,i)));
% error_phase(i)  = nmse(angle(H_UPA(:,:,i)),angle(Hinsite_NrNt(:,:,i)));
error_H(i) = nmse(H_UPA(:,:,i),Hinsite_NrNt(:,:,i))
end

%% SAVE %%
save(save_mat,'exec_time','runtime_mimo','H_UPA', 'Hinsite_NrNt', 'error_H');